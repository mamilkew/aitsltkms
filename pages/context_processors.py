...
def navbarmain(request):
    ...
    raw_query = "SELECT DISTINCT ON (subject) *, max(published_date) FROM pages_post WHERE published_date is not null GROUP BY subject, id ORDER BY subject ASC , published_date DESC"
    ...
    # print(pb_list)
    return {'navbarMain': Post.objects.raw(raw_query)}
